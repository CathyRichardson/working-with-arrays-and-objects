...
let evenNumbers = mixedNumbers.filter((num, ind, arr) => { return arr[ind] % 2 === 0})
...
//Code Here
let postTaxPrices = prices.map((price, i, arr) => {return arr[i] * 1.07});
...
//Code Here
let myStrongest = monstersInYourPocket.filter((val, i, arr) => {return arr[i]["CP"] > 200})
...
let orderTotals = orders.map((elem, i, arr) => {return arr[i]["price"] * (arr[i]["tax"] + 1) }) 
...
let bobsTotal = purchases.reduce((acc, curr, i, arr) => {
return acc + curr["owner"]["Bob"]
}, 0)
